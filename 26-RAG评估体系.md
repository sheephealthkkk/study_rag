...
#### Q1：RAG 的评估为什么需要分成"检索评估"和"生成评估"两个独立的维度？如果只看端到端的回答质量不行吗？（字节跳动 / 腾讯 高频方法论题）

**面试官考察点：** 是否理解"分维度评估"的必要性——检索好≠回答好，回答好≠检索好。

**回答框架（用三个反例讲清"为什么需要分开"）：**

**反例 1——检索好 + 生成差（"找到了但读错了"）：**

检索找到了精确的 Context "事假需提前1个工作日申请"。LLM 却回答 "事假需提前3个工作日申请"（读错了数字）。端到端评估给低分。但根因是检索还是生成？不知道。如果只看端到端，你会去调检索参数——但实际检索没问题，应该调 Prompt/temperature。

**反例 2——检索差 + 生成好（"没找到但 LLM 恰好知道"）：**

检索漏掉了相关上下文。LLM 用自己的训练知识恰好答对了。端到端评估给高分。你以为检索没问题——但下一次问 LLM 不知道的问题就翻车。检索差被"LLM 自身知识"掩盖了。

**反例 3——Context 本身错误（"照抄了错误的文档"）：**

知识库文档是旧版本，写的 "入职满2年享有10天年假"，正确的新版是 5 年。LLM 忠实回答 "入职满2年"。Faithfulness（回答 vs Context）= 高分，但 Correctness（回答 vs 标准答案）= 低分。端到端只能看到"回答错了"——分维度才能定位到"知识库文档过期了"。

**分维度评估的精准定位价值：**

```
检索低 → 调检索（Embedding/切分/混合检索）→ 不改 Prompt
生成低 → 调生成（Prompt/temperature/合成策略）→ 不改检索
端到端低 + 检索高 + 生成高 → 知识库文档有问题 → 更新知识库
端到端低 + 检索低 + 生成高 → 被"LLM 自身知识"掩盖的检索问题 → 换一批 LLM 不知道的 query 再测
```

---

#### Q2：`generate_question_context_pairs` 自动生成的评估数据集和人工标注数据集有什么区别？各有什么优缺点？如何在实际项目中结合使用？（阿里巴巴 / 美团）

**面试官考察点：** 工程实践中评估数据的构建策略——自动 vs 人工的混合方案。
...
| 维度 | 自动生成 | 人工标注 |
|------|----------|----------|
| **速度** | 100 条 ~5 分钟 | 100 条 ~4-8 小时 |
| **成本** | ~$0.50（LLM API） | ~$200-500（专家时薪） |
| **问题类型** | 偏向"事实提取"型 | 可覆盖复杂推理、多跳问题 |
| **Ground Truth** | = Node 原文（假设整个 Node 都相关） | 可精标注"哪几句是正确答案" |
| **问题自然度** | 偏书面化 | 更接近真实用户口语 |
| **覆盖率** | 每个 Node 都有对应问题 | 取决于标注范围 |

**自动生成的三个主要缺陷：**
1. 问题偏向"事实提取"——LLM 倾向于生成 "XXX是什么？" 而非 "对比 A 和 B"，生成的评估集覆盖不到复杂推理类查询
2. Ground Truth = Node 原文 → 假设整个 Node 都相关，实际上可能只有一句话是正确答案，会高估 Recall
3. 问题分布 ≠ 真实用户分布——书面语 vs 口语化

**三阶段混合策略：**

```
Phase 1（开发，0 人工）：自动生成 100 条 → 快速拿到基线 → 约 $0.50
Phase 2（调优，少量人工）：从 100 条中人工筛选 20 条 + 手写 10 条口语化 query → 约 2 小时
Phase 3（上线前，正式标注）：50 条纯人工全量标注 → 每条标注所有相关 Chunk → 约 8 小时
```
...
#### Q3：Recall、Precision、Hit Rate 三个指标分别衡量什么？为什么 Recall 的标注成本最高但不可或缺？（腾讯 / 百度 高频）

**面试官考察点：** 三个指标各自的定位和标注成本差异。

**回答框架——三个指标的本质区别是"标注范围"不同：**

```
Recall（全量标注——最贵但最全面）：
  需要标注：知识库中"所有与这条 Query 相关的 Chunk"
  标注 O(N) 成本——N=知识库 Chunk 总数
  Query="请假有哪些类型？" → 需要从头看完所有 Chunk，标注出 5 个相关的

Precision（部分标注——中成本）：
  需要标注：检索返回的 Top-K 个 Chunk 中哪些相关
  标注 O(K) 成本——K 通常 5-10

Hit Rate（最少标注——最低成本）：
  需要标注：至少 1 个相关的 Chunk（甚至可用 LLM 自动判断）
  标注 O(1) 成本
```

**为什么 Recall 最贵但不可或缺——"你不知道漏了多少"：**

```
Hit Rate=100% ≠ 系统没问题

Query="请假有哪些类型？"，正确答案需事假+病假+年假+婚假+产假（5 种）
  → 检索成功找到了"事假"的 Chunk（Hit Rate=命中）
  → 但漏掉了其他 4 种假期的 Chunk
  
  只看 Hit Rate → 认为检索没问题
  只有 Recall 能告诉你：5 个相关 Chunk 中只找回了 1 个 → Recall=20%

开发阶段用 Hit Rate 快速迭代（便宜），上线前用 Recall 做最终验证（贵但必须）。
```

---

#### Q4：MRR 和 NDCG 分别衡量什么？各自的典型使用场景是什么？（阿里巴巴 / 字节跳动）

**面试官考察点：** 有序评估两个核心指标的差异和适用场景。

**MRR vs NDCG 的本质差异：**

```
MRR（关注第一个正确答案的排名）：
  假设用户只需要一个答案——第一个正确答案的位置决定了体验
  只看"相关/不相关"（二值判断），不需要区分相关度等级
  1/rank 的数学特性：第 1→1.0, 第 2→0.5, 第 5→0.2, 第 10→0.1
  → 排名越靠后，贡献急剧下降

NDCG（关注所有相关文档的排列质量）：
  假设用户需要全面信息——所有相关文档是否都排在前面？
  需要"相关度等级"（高度相关=3、相关=2、弱相关=1）
  用 log 折扣函数对靠后的位置降权
```
...
```
用 MRR 当：用户只要一个精准答案（"年假有几天？"）、FAQ 型问答、政策查询、标注成本有限
用 NDCG 当：用户需要全面信息（"请假有哪些类型？"）、文献综述、有多级相关度标注

大多数 RAG 场景两个都要：
  MRR 告诉你"排第一的对不对"（用户体验核心）
  NDCG 告诉你"整体排序质量"（信息完整覆盖）
```
...
#### Q5：Faithfulness（忠实度）和 Factual Correctness（事实正确性）有什么区别？为什么会出现"Faithfulness 高但 Correctness 低"的情况？（字节跳动 / 腾讯 高频区分题）

**面试官考察点：** 这是最容易混淆的两个概念。

**一句话区分：Faithfulness = Answer vs Context，Correctness = Answer vs Truth。**

```
场景：知识库中是 2023 年的旧版文档，写道 "入职满2年享有10天年假"。
正确的 2024 年版应该是 "入职满5年享有10天年假"。

LLM 回答 A（忠实于旧文档）："入职满2年享有10天年假"
  Faithfulness 高（Context 中能找到原文）✓
  Correctness 低（与真实政策不一致）✗
  → 这就是 RAG 的独特风险——忠实于错误源

LLM 回答 B（编造但恰好正确）："入职满5年享有10天年假"
  Faithfulness 低（Context 中是 2 年）✗
  Correctness 高（与真实政策一致）✓
  → 危险！这次对了，下次不一定
```

**四种组合的诊断矩阵：**

| Faithfulness | Correctness | 诊断 | 优化方向 |
|:---:|:---:|------|------|
| 高 | 高 | 完美 | — |
| 高 | 低 | Context（知识库文档）本身有错 | 更新知识库、检查文档时效性 |
| 低 | 高 | LLM 在编造，恰好撞对了 | 降低 temperature、加强 Prompt 约束 |
| 低 | 低 | 全链路崩盘 | 检索+Prompt+模型 全链路排查 |

---

#### Q6：如果没有人工标注的 Ground Truth，如何评估 RAG 系统的效果？LLM-as-Judge 靠得住吗？（腾讯 / 华为 实战题）

**面试官考察点：** 现实中很少有完整 Ground Truth。

**三级替代方案（按成本递增）：**

**方案一——LLM-as-Judge 自动评估（零人工成本）：**

```
可自动评估的指标（不需要 Ground Truth）：
  Context Relevance → LLM 判断 "Chunk 是否与 Query 相关？"
  Faithfulness → LLM 判断 "Answer 是否能在 Context 中找到证据？"
  Answer Relevance → LLM 判断 "Answer 是否紧扣 Query？"

  成本：20 条 × 3 个指标 × GPT-4o ≈ $0.50
  可靠性：约 80-85%（与人工评判的一致性）
  
  注意：Correctness 必须要有 Ground Truth → LLM 自己无法判断"客观事实"
```

**方案二——合成数据集（中等成本）：**

```
generate_question_context_pairs 自动生成 QA 对 → 人工抽查 20 条修正 → 跑 Recall/Precision
局限：问题偏"事实提取"型、Ground Truth 假设整个 Node 都相关
```

**方案三——线上用户反馈（真实但延迟高）：**

```
点赞率/点踩率、复问率、转人工率、浏览深度
不是标准评估指标，但在没有 Ground Truth 时能给出方向性信号
```

**LLM-as-Judge 靠得住吗——"能用但有盲区"：**

```
靠得住：判断 Chunk-Query 相关性（二值判断）、判断 Claim 是否有 Context 证据、做粗筛再人工抽检
靠不住：判断客观事实是否正确、判断微妙语义差异、一致性差（同一输入两次可能不同分）

最佳实践：LLM-as-Judge 预筛 + 人工抽查 10-20% 校准
  → LLM 与人工一致率 > 85% → 信赖 LLM 评判
  → 一致率 < 70% → 优化评估 Prompt 或改人工评估
```
...
| 问题 | 一句话答案 |
|------|-----------|
| 为什么评估要分检索和生成两个维度？ | 检索好≠回答好，回答好≠检索好——分开评估才能精准定位故障环节 |
| Recall 为什么标注成本最高？ | 需要标注知识库中"所有"相关 Chunk（O(N)），而 Precision 只需标注返回的 Top-K（O(K)） |
| Hit Rate 为什么最实用？ | 标注成本最低（只需 1 个相关 Chunk），可以直接反映"多少查询能找到答案" |
| MRR 和 NDCG 的本质区别？ | MRR 只看第一个正确答案的位置，NDCG 看所有相关文档的排列质量 |
| Faithfulness 和 Correctness 的区别？ | Faithfulness = Answer vs Context（有没有编？），Correctness = Answer vs Truth（对不对？） |
| Faithfulness 高 + Correctness 低说明什么？ | 知识库文档本身有错——RAG 的独特风险：忠实于错误源 |
| 没有 Ground Truth 怎么评估？ | LLM-as-Judge 自动评估 + 合成数据集 + 线上反馈，三级递进 |
| 评估的推荐顺序？ | Context Relevance（零成本）→ Hit Rate（LLM 自动）→ Precision（少量标注）→ Recall（全量标注，上线前） |

#### 评估指标全景速查表

| 指标 | 维度 | 标注成本 | 衡量什么 | 低分指向 | LLM自动？ |
|------|:---:|:---:|------|------|:---:|
| **Recall** | 检索 | 最高（全量） | 找得全不全 | 漏检/chunk 策略 | ✗ |
| **Precision** | 检索 | 中（K个） | 找得准不准 | 噪声/Rerank 缺失 | ✗ |
| **Hit Rate** | 检索 | 最低（1个） | 找不找得到 | 知识库覆盖/Embedding | ✓(LLM) |
| **MRR** | 检索 | 低 | 第一个排第几 | 排序质量/Reranker | ✗ |
| **NDCG** | 检索 | 高（需等级） | 整体排序质量 | 所有相关的排列 | ✗ |
| **Context Relevance** | 检索 | 零 | 检索是否有用 | 检索返回噪声 | ✓(LLM) |
| **Faithfulness** | 生成 | 零 | 有没有编造 | Prompt/temperature | ✓(LLM) |
| **Answer Relevance** | 生成 | 零 | 有没有跑题 | 检索噪声 | ✓(LLM) |
| **Correctness** | 生成 | 最高 | 事实对不对 | 知识库/文档本身错 | ✗(需GT) |
